...
//     pdf.text("ENROLLMENT FORM", 105, 67, { align: "center", color: "gray" });



//     // Set Y-coordinate for the content after the image
//     let yCoordinate = 60;

//     // Add content to the PDF document
//     // pdf.text("Student Details", 20, yCoordinate);
//     // Increase Y-coordinate for each line to create a systematic layout
//     yCoordinate += 10;



//     // Add content to the PDF document
//     pdf.addImage(imageDataUrl, "JPEG", 130, 73, 40, 40);
//     pdf.text(`Enrollement No: 156757653465`, 20, (yCoordinate += 10));
//     pdf.text(`Name of Candidate: ${userData.name}`, 20, (yCoordinate += 10));
//     pdf.text(`Father's Name: ${userData.fathersname}`, 20, (yCoordinate += 10));
//     pdf.text(`Mother's Name: ${userData.mothersname}`, 20, (yCoordinate += 10));
//     pdf.text(`Gender: ${userData.gender}`, 20, (yCoordinate += 10));
//     pdf.text(`Date of Birth: ${userData.dob}`, 20, (yCoordinate += 10));
//     pdf.text(
//       `Address: ${userData.address.address2}`,
//       20,
//       (yCoordinate += 10)
//     );
//     pdf.text(`Domicile: ${userData.domicile}`, 20, (yCoordinate += 10));
//     pdf.text(`Category: ${userData.category}`, 20, (yCoordinate += 10));

//     pdf.text(`Mobile No.: ${userData.mobile}`, 20, (yCoordinate += 10));

//     pdf.text(`Email: ${userData.email}`, 20, (yCoordinate += 10));


//     pdf.text(`Course Name: ${userData.courseName}`, 20, (yCoordinate += 10), { bold: true });
//     pdf.text(`Course Type: ${userData.courseType}`, 20, (yCoordinate += 10), { bold: true });
//     pdf.text(`Specialization: ${userData.courseBranch}`, 20, (yCoordinate += 10), { bold: true });
//     pdf.text(`Institute: ${userData.assignedCollege}`, 20, (yCoordinate += 10), { bold: true });
//     pdf.text(`Mode of Admission: University Level`, 20, (yCoordinate += 10));
...
//     yCoordinate = 10; // Reset yCoordinate for new page
//     pdf.setFont("helvetica", "bold");
//     pdf.text("EDUCATION DETAILS:-", 20, (yCoordinate += 10));
...
//     ];
//     // Set column widths
//     const colWidths = [40, 50, 22, 22, 22, 22];
//     const tableOptions = {
//       startY: 30,
//       startX: 20,
//       margin: { top: 10 },
...
//     pdf.autoTable(educationHeaders, educationData, tableOptions);

//     // Save the PDF
//     pdf.save("student_details.pdf");


//     // Display the download link for the image
...
//           </Grid>
//           <Button onClick={generatePDF} variant="warning">Download Now</Button>
//         </Box>
//       </ThemeProvider>
//     </>
//   );
...
function StudentAllDetail() {
      ...
      if (userData.IsEnrollGenerated === true) {
       ...
       // setIsEnrollGenerated(false)
        swal({
          icon: 'warning',
          title: 'Contact Admin',
          text: 'Please contact the admin to generate the enrollment form.',
        })
      }
  ...
  const generatePDF = async () => {
    ...
    // pdf.text("Student Details", 20, yCoordinate, { bold: true });


    // Add content to the PDF document
    // pdf.addImage(imageDataUrl, "JPEG", 10, yCoordinate-7, 40, 40);
    ...
    // pdf.text(`Mother's Name: ${userData.mothersname}`, 20, (yCoordinate += 10));
    // pdf.text(`Gender: ${userData.gender}`, 20, (yCoordinate += 10));
    // pdf.text(`Date of Birth: ${formatDate(userData.dob)}`, 20, (yCoordinate += 10));
    // pdf.text(
    //   `Address: ${userData.address.address2}`,
    //   20,
    //   (yCoordinate += 10)
    ...
    // Adjust startY and margin.top to display below the first table
    const educationOptions = {
      startY: 210, // Adjust this value based on the height of the first table
      startX: 20,
      margin: { top: 40 }, // Adjust the top margin for spacing
      columnStyles: { 0: { cellWidth: colWidths[0] }, 1: { cellWidth: colWidths[1] }, 2: { cellWidth: colWidths[2] }, 3: { cellWidth: colWidths[3] }, 4: { cellWidth: colWidths[4] }, 5: { cellWidth: colWidths[5] } },
      styles: { overflow: "linebreak" },
    };
        ...
        >
          <Grid container sx={{ justifyContent: "center", }}>
            <Grid item xs={12} sm={5} >
              ...
              <div >
                <Button variant="contained" color="warning"><Link to='/studentlogin'>Logout</Link></Button>
                      ...
                      <div class="card-body pt-0">
                        ...
                        >
                          ...
                          <tr>
                            ...
                            <td width="2%">:</td>
                            <td>{userData.professional.ScholarshipRequired}Not Eligible </td>
                          </tr>
